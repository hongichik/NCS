...
def main():
    ...
    data_dir = REPO_ROOT / 'Data' / 'COTREC' / opt.dataset.lower()
    train_data = pickle.load(open(data_dir / 'train.txt', 'rb'))
    test_data = pickle.load(open(data_dir / 'test.txt', 'rb'))
    all_train = pickle.load(open(data_dir / 'all_train_seq.txt', 'rb'))
    if opt.dataset == 'diginetica':
        n_node = 43097
    elif opt.dataset == 'Tmall':
        n_node = 40727
    elif opt.dataset in ['retailrocket', 'RetailRocket']:
        n_node = 36968
    else:
        n_node = 309
    ...
    write_run_summary(
        ...
        opt.dataset.lower(),
        format_best_k_summary(opt.dataset, best_results, top_K, header=f"model=COTREC | {opt}"),
    )
