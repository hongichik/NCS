...
class LocalAggregator(nn.Module):
    ...
    def forward(self, hidden, adj, mask_item=None):
        ...
        # Efficient attention: e[b,i,j] = leakyrelu(sum_d(h[b,i,d]*h[b,j,d]*a[d]))
        # Equivalent to: leakyrelu(bmm(h * a.T, h.T)) -- avoids O(N^2*dim) tensor
        e_0 = self.leakyrelu(torch.bmm(h * self.a_0.t(), h.transpose(1, 2)))
        e_1 = self.leakyrelu(torch.bmm(h * self.a_1.t(), h.transpose(1, 2)))
        e_2 = self.leakyrelu(torch.bmm(h * self.a_2.t(), h.transpose(1, 2)))
        e_3 = self.leakyrelu(torch.bmm(h * self.a_3.t(), h.transpose(1, 2)))
