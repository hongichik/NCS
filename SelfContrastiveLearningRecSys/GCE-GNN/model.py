...
class CombineGraph(Module):
    ...
    def compute_scores(self, hidden, mask, use_item_cl_loss):
        ...
        pos_idx = torch.arange(seq_len, device=hidden.device) % self.pos_embedding.num_embeddings
...
def forward(model, data, use_item_cl_loss):
    ...
    # Vectorized gather: hidden[b, alias_inputs[b, s]] for all b, s
    seq_hidden = hidden[torch.arange(hidden.shape[0]).unsqueeze(1), alias_inputs]
...
def train_test(model, train_data, test_data, cl_loss_function, use_item_cl_loss=False, sampled_item_size=30000, temperature=0.1, item_cl_loss_weight=0.1, saved_models_path=None):
    ...
    for data in tqdm(train_loader):
        ...
        if use_item_cl_loss:
            ...
            sample_size = min(sampled_item_size, bs)
            sampled_indices = torch.randperm(bs, device=item_embeddings.device)[:sample_size]
            sampled_item_embeddings = item_embeddings[sampled_indices]
            logits = torch.div(
                torch.matmul(sampled_item_embeddings, sampled_item_embeddings.T), temperature)
            targets_cl = torch.arange(sample_size, device=item_embeddings.device)
            cl_item_loss = cl_loss_function(logits, targets_cl)
