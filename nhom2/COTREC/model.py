...
class COTREC(Module):
    def __init__(self, adjacency, n_node, lr, layers, l2, beta,lam,eps, dataset, emb_size=100, batch_size=100):
        ...
        self.adv_item = torch.zeros((self.n_node, self.emb_size), device=self.embedding.weight.device).requires_grad_(True)
        self.adv_sess = torch.zeros((self.n_node, self.emb_size), device=self.embedding.weight.device).requires_grad_(True)
    ...
    def generate_sess_emb(self, item_embedding, session_item, session_len, reversed_sess_item, mask):
        ...
        seq_h = torch.zeros((self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size), device=item_embedding.device)
    ...
    def adversarial_item(self, item_emb, tar,sess_emb):
        if self.adv_item.device != item_emb.device:
            self.adv_item = self.adv_item.to(item_emb.device)
    ...
    def adversarial_sess(self, item_emb, tar,sess_emb):
        if self.adv_sess.device != item_emb.device:
            self.adv_sess = self.adv_sess.to(item_emb.device)
    ...
    def topk_func_random(self, score1,score2, item_emb_I, item_emb_S):
        ...
        device = item_emb_I.device
        pos_emb_I = torch.zeros((self.K, self.batch_size, self.emb_size), device=device)
        pos_emb_S = torch.zeros((self.K, self.batch_size, self.emb_size), device=device)
        neg_emb_I = torch.zeros((self.K, self.batch_size, self.emb_size), device=device)
        neg_emb_S = torch.zeros((self.K, self.batch_size, self.emb_size), device=device)
...
def train_test(model, train_data, test_data, epoch):
    ...
    n_slices = len(slices)
    interval = max(1, n_slices // 100)
    start_time = time.time()
    for batch_idx, i in enumerate(slices, start=1):
        model.zero_grad()
        ...
        if batch_idx % interval == 0 or batch_idx == n_slices:
            elapsed = time.time() - start_time
            avg_time = elapsed / batch_idx
            eta = avg_time * (n_slices - batch_idx)
            print('train progress:\t%d/%d (%.1f%%)\tavg_loss: %.4f\teta: %.1fs' %
                  (batch_idx, n_slices, 100.0 * batch_idx / n_slices, total_loss / batch_idx, eta), flush=True)
