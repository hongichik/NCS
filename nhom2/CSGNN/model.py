...
class HyperConv(Module):
    ...
    def forward(self, adjacency, embedding):
        ...
        adjacency = torch.sparse_coo_tensor(i, v, torch.Size(shape)).coalesce()
        ...
        item_embeddings = torch.stack(final, dim=0).sum(dim=0)
...
class LineConv(Module):
    ...
    def forward(self, item_embedding, D, A, session_item, session_len, session_item_cat):
        zeros = item_embedding.new_zeros((1, self.emb_size))
        ...
        session_emb_lgcn = torch.stack(session, dim=0).sum(dim=0)
...
class DHCN(Module):
    ...
    def generate_sess_emb(self, item_embedding, cat_embedding, session_item, session_item_cat, session_len,
        ...
        seq_h = torch.zeros(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size, device=device)
        seq_l = torch.zeros(self.batch_size, self.K, self.emb_size, device=device)
        seq_h_cat = torch.zeros(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size, device=device)
