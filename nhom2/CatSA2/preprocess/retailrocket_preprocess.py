"""
Preprocess RetailRocket for CatSA2.

Same protocol as CatSA / SR-GNN / COTREC baseline.
Outputs to Data/CatSA2/retailrocket/.
"""
...
def remap_category_maps(
    ...
    """Remap to 0-based item idx; lookup tables use train+valid only (no leakage)."""
    item2cat: dict[int, int] = {}
    for raw_item, leaf in raw_item2leaf.items():
        if raw_item not in raw_to_idx:
            continue
        idx = raw_to_idx[raw_item] - 1
...
def configure_logging(log_dir: Path, log_file_name: str) -> logging.Logger:
    logger = logging.getLogger("catsa2.preprocess")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Preprocess RetailRocket for CatSA2.")
