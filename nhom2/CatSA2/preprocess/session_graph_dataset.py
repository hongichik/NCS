...
class CategorySessionGraphDataset(Dataset[HeteroData]):
    def __init__(
        ...
        self._graphs = []
        self._raw_kept: list[list[int]] = []
        kept_targets: list[int] = []
        for i, seq in enumerate(self.session_sequences):
            target = None if self.targets is None else self.targets[i]
            try:
                graph = self._build_graph(seq, target)
            except ValueError:
                continue
            self._graphs.append(graph)
            self._raw_kept.append(self.raw_sessions[i])
            if target is not None:
                kept_targets.append(target)
        if self.targets is not None:
            self.targets = kept_targets
        self.raw_sessions = self._raw_kept
        self.session_sequences = [
            list(map(int, s)) for s in self._raw_kept
        ]
