...
class COTREC(Module):
    ...
    def generate_sess_emb(self, item_embedding, session_item, session_len, reversed_sess_item, mask):
        ...
        len = seq_h.shape[1]
        pos_idx = torch.arange(len, device=item_embedding.device) % self.pos_embedding.num_embeddings
    ...
    def adversarial_item(self, item_emb, tar, sess_emb):
        if self.adv_item is None or self.adv_item.shape != item_emb.shape or self.adv_item.device != item_emb.device:
            self.adv_item = torch.zeros_like(item_emb, requires_grad=True)
    ...
    def adversarial_sess(self, item_emb, tar, sess_emb):
        if self.adv_sess is None or self.adv_sess.shape != item_emb.shape or self.adv_sess.device != item_emb.device:
            self.adv_sess = torch.zeros_like(item_emb, requires_grad=True)
