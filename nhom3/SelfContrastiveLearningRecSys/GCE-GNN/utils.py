...
class Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Build a lookup dict for O(1) index retrieval instead of O(n) np.where scans
        node_to_idx = {n: i for i, n in enumerate(node)}
        adj = np.zeros((max_n_node, max_n_node))
        for i in range(len(u_input) - 1):
            u = node_to_idx[u_input[i]]
            adj[u][u] = 1
            if u_input[i + 1] == 0:
                break
            v = node_to_idx[u_input[i + 1]]
            if u == v or adj[u][v] == 4:
                continue
            adj[v][v] = 1
            if adj[v][u] == 2:
        ...
        alias_inputs = [node_to_idx[i] for i in u_input]
