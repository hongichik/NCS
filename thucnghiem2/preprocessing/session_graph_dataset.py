...
class EncodedTaxonomy:
    """Contiguous vocabularies and taxonomy edges used by the session graph dataset."""
...
def build_prefix_target_pairs(session_sequences: Sequence[Sequence[int]]) -> tuple[list[list[int]], list[int]]:
    """
    Expand full sessions into prefix-target training pairs.

    Example:
    [1, 2, 5] -> prefixes [[1], [1, 2]], targets [2, 5]
    """
...
def fit_taxonomy_encoders(
    ...
    """Map raw ids to contiguous indices for embedding lookup."""
...
class CategorySessionGraphDataset(Dataset[HeteroData]):
    """
    Build one heterogeneous session graph per session prefix.

    Each graph contains:
    - item nodes linked by sequential edges.
    - item <-> leaf_cat bipartite edges.
    - leaf_cat <-> parent_cat taxonomy edges.
    """
    ...
    def __init__(
        ...
        dict_same_leaf: Mapping[int, Sequence[int]] | None = None,
        dict_sibling: Mapping[int, Sequence[int]] | None = None,
    ) -> None:
        self.session_sequences = [list(map(int, session)) for session in session_sequences if len(session) >= 1]
        ...
        if self.targets is not None and len(self.targets) != len(self.session_sequences):
            raise ValueError("targets must have the same length as session_sequences")
            
        self.dict_same_leaf = dict_same_leaf
        ...
        self._graphs = [
            self._build_graph(session, None if self.targets is None else self.targets[index])
            for index, session in enumerate(self.session_sequences)
        ]
    ...
    def __getitem__(self, index: int):
        base_graph = self._graphs[index]
        if self.dict_same_leaf is None or self.dict_sibling is None:
            return base_graph

        import random

        raw_session = self.session_sequences[index]
        target = None if self.targets is None else self.targets[index]

        # View 1: Same-Leaf Substitution
        view1_session = list(raw_session)
        replace_idx = random.randint(0, len(raw_session) - 1)
        item_to_replace = raw_session[replace_idx]
        ...
        replace_idx2 = random.randint(0, len(raw_session) - 1)
        item_to_replace2 = raw_session[replace_idx2]
    ...
    def _build_graph(self, raw_session: Sequence[int], target: int | None) -> HeteroData:
        encoded_session = [self.taxonomy.item_encoder[item_id] for item_id in raw_session if item_id in self.taxonomy.item_encoder]
        if not encoded_session:
            raise ValueError("Session contains no items present in the fitted item vocabulary")
        ...
        leaf_parent_src = [src for src, _ in leaf_to_parent_edges]
        leaf_parent_dst = [dst for _, dst in leaf_to_parent_edges]
        ...
        if target is not None:
            ...
            if encoded_target is None:
                raise ValueError(f"Target item {target} is missing from the item vocabulary")
